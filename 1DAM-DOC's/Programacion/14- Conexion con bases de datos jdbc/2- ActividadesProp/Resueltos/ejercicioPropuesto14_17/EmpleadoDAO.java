...
public class EmpleadoDAO {
		 ...
		 if(objEmpleado!=null) {
			 if((con = conecta())!=null)
				 try {
					...
					/*ResultSet rs = st.executeQuery("select distinct emp_no from emple where emp_no=" + objEmpleado.getIntEMP_NO());
					if(rs.next())
	 ...
	 public static Empleado busca(int intEMP_NO, Connection con) {
		 ...
		 if(con!=null)
			 try {
					Statement st = con.createStatement();
					System.out.println("select * from emple where emp_no=" + intEMP_NO);
